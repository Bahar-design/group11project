...
describe('Volunteer History API', () => {
  ...
  it('POST /api/volunteer-history creates a new volunteer record', async () => {
    const volunteerRecord = {
      volunteer_id: VALID_VOLUNTEER_ID,
    ...
    expect(res.body.volunteer_id).toBe(VALID_VOLUNTEER_ID);
    expect(res.body.event_id).toBe(VALID_EVENT_ID);
  });
  ...
  it('PUT /api/volunteer-history/:id updates a record', async () => {
    const create = await request(app)
      .post('/api/volunteer-history')
      .send({ volunteer_id: VALID_VOLUNTEER_ID, event_id: VALID_EVENT_ID });

    const id = create.body.history_id;

    const updateRes = await request(app)
      .put(`/api/volunteer-history/${id}`)
      .send({ volunteer_id: 9 }); // another valid volunteer_id

    expect(updateRes.statusCode).toBe(200);
    expect(updateRes.body.volunteer_id).toBe(9);
  });
  ...
  it('DELETE /api/volunteer-history/:id deletes a record', async () => {
    ...
    const resDel = await request(app).delete(`/api/volunteer-history/${id}`);

    expect(resDel.statusCode).toBe(200);
    expect(resDel.body.history_id).toBe(id);
