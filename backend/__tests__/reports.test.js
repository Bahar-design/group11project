...
describe('Reports endpoints', () => {
  ...
  test('GET /api/reports/volunteer-participation returns rows', async () => {
    pool.query.mockResolvedValueOnce({ rows: [ { volunteer_id: 1, full_name: 'A', email: 'a@x.com', city: 'X', state_code: 'TX', total_events: '2', total_hours: '5', skills: ['S1'] } ] });
  ...
  test('GET /api/reports/volunteer-history returns rows', async () => {
    pool.query.mockResolvedValueOnce({ rows: [ { volunteer_id: 1, full_name: 'A', event_id: 10, event_name: 'E', event_date: '2025-10-10', hours_worked: '3' } ] });
    const res = await request(app).get('/api/reports/volunteer-history');
    expect(res.statusCode).toBe(200);
    expect(res.body[0].hours_worked).toBe(3);
  });

  test('GET /api/reports/event-management returns rows', async () => {
    pool.query.mockResolvedValueOnce({ rows: [ { event_id: 10, event_name: 'E', total_volunteers: '4', total_hours: '12', required_skills: ['S1'] } ] });
